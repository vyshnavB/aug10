...
def intrest_page(request):
    ...
    intr_followers = intrest_followers.objects.filter(following_user=following_user)
    intr_followers_ids = intr_followers.values_list('topic_id', flat=True)  # Get the IDs of the topics the user is following
    
    if query:
        # If there is a search query, filter the interests by 'intrest_name' containing the query
        intr = intr.filter(intrest_name__icontains=query)
        return render(request, "search_results.html", {"intr": intr})
    
    return render(request, "intrest.html", {"intr": intr, "intr_followers": intr_followers_ids})
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if selected_value == "best" :
            ...
            unread_notification_count = Notifications.objects.filter(from_user=request.user, is_read=False).count()
...
def profile(request, username):
    ...
    car=Cart.objects.filter(product__creater=username)
    ...
    noification_count=Notifications.objects.filter(to_user=request.user).count()
    ...
    context = {
        ...
        "unread_notification_count":unread_notification_count,
        "frd":frd,
        "noification_count":noification_count,
        'crt' : crt,
        'crt_count' : crt_count,
        "invc":inv.count(),  
...
from django.views.decorators.csrf import csrf_exempt
from .models import intrest, Post
...
def user_create_post(request, pk):
    if request.method == 'POST':
        ...
        # Add any necessary context data for rendering the template
        return render(request, 'profile.html', context_data)



@csrf_exempt
def user_create_post_organization(request,pk):
    if request.method == 'POST':
...
def pageprofile(request,pageid):
    ...
    user_followers = len(friend_request.objects.filter(to_user=pageid))
    user_following = len(friend_request.objects.filter(from_user=pageid))   
    intrests=intrest.objects.all().order_by("?")[:5]    
...
def pageprofile2(request,pageid):
    ...
    if friend_request.objects.filter(from_user__id=to, to_page__id=frm,stat='following').first():
            button_text = 'Unfollow'
    else:  
            button_text = 'Follow' 

    user_followers = len(friend_request.objects.filter(to_page=frm))
    ...
    if request.user.is_authenticated:
        joined_users = friend_request.objects.filter(to_page=pro, stat="following").values_list("from_user", flat=True)
        pending_users = friend_request.objects.filter(to_page=pro, stat="Pending").values_list("from_user", flat=True)
...
def create_pagepost(request,pageid):
    ...
    if request.method == 'POST':
        ...
        post.save()
        return redirect("pageprofile2",pageid)
...
#         'crt_count' : crt_count,
#         "invc":inv.count(), "intr": intr, "intr_followers": intr_followers_ids })
...
def edit_profile(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Process the cropped image data as needed, e.g., save it to a file or database
        # For example, you can use PIL (Pillow) to save the image to a file on the server
        # Example:
        # from PIL import Image
        # img = Image.open(cropped_image_data)
        # img.save('path/to/destination/image.jpg')

        # Return a response indicating success
        return HttpResponse('Image cropped and saved successfully!')
    ...
    following_user = request.user
    intr_followers = intrest_followers.objects.filter(following_user=following_user)
    intr_followers_ids = intr_followers.values_list('topic_id', flat=True)
    ...
    return render(request, "edit_profile.html", {
        ...
        "invc": inv.count(),
        "intr": intr,
        "intr_followers": intr_followers_ids
    })
...
def save_cropped_image(request, pk):
    if request.method == 'POST':
        try:
            profile = User.objects.get(id=pk)
            # Process the cropped image file and save it as the new profile picture
            cropped_image = request.FILES.get('cropped_image')
            if cropped_image:
                profile.profile_pic = cropped_image
                profile.save()
...
def intrest_follow_editprofile(request,pk,id):
    followig_user=request.user
    topic=intrest.objects.get(id=pk)
    intr_follow=intrest_followers(following_user=followig_user,topic=topic,status="following")
    intr_follow.save()
    return redirect("edit_profile",id)


@csrf_exempt
def intrest_unfollow_editprofile(request,pk,id):
    followig_user=request.user
    topic=pk
    s=intrest_followers.objects.filter(following_user=followig_user,topic=topic,status="following")
    s.delete()
    return redirect("edit_profile",id)
...
def sent_friend_request_org(request,userid):
    from_user=request.user
    to_user=User.objects.get(id=userid)
    frequest =friend_request(from_user=from_user,to_user=to_user,stat="following")
    frequest.save()
...
def sent_invite_request(request,userid,id):
    ...
    frequest.to_page=pages
    frequest.from_user=to_user
    frequest.to_user=from_user
    frequest.stat="Pending"
    frequest.save()

    user_page_noti = Notifications()

    inv_id = friend_request.objects.get(id=frequest.id)

    user_page_noti.pages = pages
    user_page_noti.to_user = to_user
    user_page_noti.from_user = from_user

    user_page_noti.friend_request = inv_id
...
def page_follow(request,userid):
    ...
    frequest =friend_request(from_user=from_user,to_page=to_page,stat="following")
    ...
    follow_Notification.from_user = request.user
    follow_Notification.to_page = to_page
    follow_Notification.friend_request = friend_request_id
    follow_Notification.type = "page_Following"
    follow_Notification.save()


    return redirect("pageprofile2",userid) 
...
def page_unfollow(request,pk):
    ...
    s=friend_request.objects.filter(from_user=to,to_page=fr,stat="following")
    s.delete()
...
def create_post(request, pk):
    # Retrieve all interest followers
    choose = intrest_followers.objects.all()

    # Retrieve all interests
    intu = intrest.objects.all()

    # Retrieve the user's cart and calculate the cart count
    crt = Cart.objects.filter(user=request.user)
    crt_count = crt.count()

    # Retrieve invite requests for the current user
    inv = invite_request.objects.filter(to_user=request.user)

    # Get the first interest (for demonstration purposes)
    parent_interest = intu.first()

    if parent_interest is not None:
        # Function to get the path of parent interests for a given interest
        def get_interest_path(interest, path=[]):
            if interest.parent:
                path.insert(0, interest.parent.id)
                get_interest_path(interest.parent, path)
        ...
        # Function to get all sub-level interests for a given parent interest
        def get_sublevel_interests(parent):
            sublevels = parent.intrest_set.all()
            sublevel_interests = []
            for sublevel in sublevels:
        ...
        # Get the path of parent interests and sub-level interests
        parent_path = get_interest_path(parent_interest)
        parent_path_2 = [intrest.objects.get(id=i) for i in parent_path]
        sublevels = get_sublevel_interests(parent_interest)
        sublevels_2 = [intrest.objects.get(id=i) for i in sublevels]
    else:
        parent_path = []
        parent_path_2 = []
        sublevels = []
    ...
    # Retrieve all interest followers
    intrest_following = intrest_followers.objects.all()
    intrest_objects = intrest.objects.all()
    categories = item_categories.objects.all()
    ...
    context = {
        ...
        "sublevels_2": sublevels_2,
        "intrest_following": intrest_following,
        'intrest_objects': intrest_objects,
        'categories': categories,
...
def Page_notifications(request,id):
    ...
    Notification =  Notifications.objects.filter(to_page__creater=request.user)
...
def Accept_notifiction_User(request,id,pk):
    ...
    pge=inv.pages
    fr_usr=inv.from_user
    to_usr=inv.to_user

    inv.save()

    frnd=friend_request(from_user=fr_usr,to_user=to_usr,to_page=pge,stat="following")
...
def user_Accept_userpage_notifiction(request,id,pk):
    ...
    inv = friend_request.objects.get(id=id)
...
def wishlist_users(request):
    ...
    item_categ = item_categories.objects.all()
...
def item_category_create(request):
    if request.method == 'POST':
        # Get the item_categories data from the form submission
        item_categories_data = request.POST.get('item_categories')

        # Create a new item_categories instance and save it to the database
        new_item_category = item_categories(item_categories=item_categories_data)
        new_item_category.save()

        # Redirect to the appropriate page after successful submission
        return redirect('item_category_create')  # Assuming you have a URL named 'item_category_list' to redirect after successful submission

    return render(request, "item_category_create.html")



def intrest_create(request):
    if request.method == 'POST':
        item_categories_id = request.POST.get("item_categories")
        ...
        if intrest.objects.filter(intrest_name=intrest_name).exists():
            messages.error(request, "An interest with that name already exists.")
            return redirect("wishlist_users")

        if parent != "":
            intrest1 = intrest.objects.get(id=parent)

            temp = []
            temp2 = []

            parent1 = intrest1.parent_id

            temp.append(intrest1.parent_id)
            temp2.append(intrest1.parent)
            while parent1 is not None:
                intrest3 = intrest.objects.get(id=parent1)
                if intrest3.parent_id is not None:
                    temp.append(intrest3.parent_id)
                    temp2.append(intrest3.parent)
                    parent1 = intrest3.parent_id
                else:
                    parent1 = None
            ...
            intrest2 = intrest()
            intrest2.item_categories_id = item_categories_id  # Use 'item_categories_id' instead of 'item_categories'
            intrest2.intrest_name = intrest_name
            intrest2.tooltip = tooltip
            intrest2.level = level
            intrest2.tooltip2 = tooltip2

            intrest2.parent_id = parent
            intrest2.save()

            return redirect("wishlist_users")
        else:
            intrest_create = intrest()
            intrest_create.item_categories_id = item_categories_id  # Use 'item_categories_id' instead of 'item_categories'
...
def convert_to_excel(request):
    # Retrieve the relevant orders for the user
    user = User.objects.get(username=request.user.username)
    car = Cart.objects.filter(product__creater=user)
    ...
    headers = ['Created At', 'Product Price', 'Status', 'User']
    for col_num, header in enumerate(headers, 1):
        col_letter = get_column_letter(col_num)
        sheet[f'{col_letter}1'] = header
        sheet[f'{col_letter}1'].font = Font(bold=True)

    # Write order details to the Excel sheet
    for row_num, order in enumerate(car, 2):
        created_at = order.created_at.astimezone(timezone.utc).replace(tzinfo=None)  # Convert to timezone-naive datetime
        sheet[f'A{row_num}'] = created_at
        sheet[f'B{row_num}'] = order.product.Product_Price
        sheet[f'C{row_num}'] = order.status
        sheet[f'D{row_num}'] = order.user.username
...
def pageadmin(request):
    ...
    def get_intrest_path(intrest_obj):
        path = []
        current_intrest = intrest_obj

        # Traverse the parent-child relationship until there is no parent
        while current_intrest:
            path.insert(0, current_intrest.intrest_name)
            current_intrest = current_intrest.parent

        return ' / '.join(path)

    topics = []
    for intrest_obj in intrests:
        path = get_intrest_path(intrest_obj)
        topics.append((intrest_obj, path))
    ...
    if request.method == 'POST' and request.FILES.get('excel_file'):
        ...
        try:
            existing_file = ExcelData.objects.first()
            if existing_file:
                os.remove(existing_file.uploaded_file.path)
                existing_file.delete()
        except:
            pass

        # Save the uploaded Excel file to the ExcelData model
        excel_data = ExcelData(uploaded_file=excel_file)
        excel_data.save()
        ...
        return render(request, 'admin/pageadmin.html', {'headers': headers, 'excel_data': excel_data, 'intrests': intrests, 'posts': posts, 'topics': topics})

    return render(request, 'admin/pageadmin.html', {'users': users, 'posts': posts, 'topics': topics})




def download_excel(request, excel_data_id):
    excel_data = get_object_or_404(ExcelData, id=excel_data_id)
    excel_file_path = excel_data.uploaded_file.path

    with open(excel_file_path, 'rb') as file:
        response = HttpResponse(file.read(), content_type='application/vnd.ms-excel')
        response['Content-Disposition'] = 'attachment; filename=uploaded_file.xlsx'
        return response
